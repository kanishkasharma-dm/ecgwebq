  ...
  try {
    ...
    const filesWithUrls = await Promise.all(
      paginatedObjects.map(async (obj: S3Object) => {
        // Extract record ID from key using filename without extension
        // Avoid including date folders like 2026/01/28 in the recordId
        const key = obj.Key || '';
        const filename = key.split('/').pop() || key;
        const recordId = filename.replace(/\.(pdf|json)$/i, '');
