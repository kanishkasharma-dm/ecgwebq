...
export default function SMode() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('report_source_mode', 'BiPAP S Mode');
    } catch {}
  }, []);
  ...
  const [ipap, setIpap] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_ipap') : null;
    return v ? Number(v) : defaults.ipap;
  });
  const [epap, setEpap] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_epap') : null;
    return v ? Number(v) : defaults.epap;
  });
  const [startEpap, setStartEpap] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_startEpap') : null;
    return v ? Number(v) : defaults.startEpap;
  });
  const [tiMin, setTiMin] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_tiMin') : null;
    return v ? Number(v) : defaults.tiMin;
  });
  const [tiMax, setTiMax] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_tiMax') : null;
    return v ? Number(v) : defaults.tiMax;
  });
  const [riseTime, setRiseTime] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_riseTime') : null;
    return v ? Number(v) : defaults.riseTime;
  });
  const [sensitivity, setSensitivity] = useState(() => {
    const v = typeof window !== 'undefined' ? localStorage.getItem('smode_sensitivity') : null;
  ...
  const [lastSaved, setLastSaved] = useState<number | null>(() => {
      const v = typeof window !== 'undefined' ? localStorage.getItem('smode_lastSaved') : null;
  ...
  const saveAll = () => {
    try {
      localStorage.setItem('smode_ipap', String(ipap));
      localStorage.setItem('smode_epap', String(epap));
      localStorage.setItem('smode_startEpap', String(startEpap));
      localStorage.setItem('smode_tiMin', String(tiMin));
      localStorage.setItem('smode_tiMax', String(tiMax));
      localStorage.setItem('smode_riseTime', String(riseTime));
      localStorage.setItem('smode_sensitivity', String(sensitivity));
      const now = Date.now();
      localStorage.setItem('smode_lastSaved', String(now));
      setLastSaved(now);
      setStatus('saved');
  ...
  const resetAll = () => {
    ...
    try {
      localStorage.removeItem('smode_ipap');
      localStorage.removeItem('smode_epap');
      localStorage.removeItem('smode_startEpap');
      localStorage.removeItem('smode_tiMin');
      localStorage.removeItem('smode_tiMax');
      localStorage.removeItem('smode_riseTime');
      localStorage.removeItem('smode_sensitivity');
      localStorage.removeItem('smode_lastSaved');
    } catch {}
    setLastSaved(null);
    setStatus('reset');
    setTimeout(() => setStatus('idle'), 2000);
    ...
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-800'}`}>
      <aside className={`w-64 backdrop-blur-md border-r fixed h-full z-20 hidden md:flex flex-col transition-colors duration-300 ${
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          {[
            ...
            { name: 'S Mode', icon: Activity, active: true, to: '/cpap/s_mode' },
      ...
      <main className="flex-1 md:ml-64 p-8 transition-colors duration-300">
        <header className="flex items-center justify-between mb-8">
          <div>
            <h1 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>BiPAP S Mode</h1>
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm font-medium text-slate-500 bg-teal-100 dark:bg-teal-900/30 dark:text-teal-400 text-teal-600 px-2 py-0.5 rounded-md">Current Mode: S Mode</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className={`font-bold ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>S Mode Settings</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className={`rounded-2xl p-4 border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm font-semibold text-slate-700 dark:text-white">{riseTime}</div>
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <button
                    className={`px-3 py-2 rounded-xl ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-700'} border ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}
                    onClick={() => setRiseTime(l => clamp(l - 1, 1, 6))}
                  >
                    <ChevronDown size={18} />
                  </button>
                  <input
                    ...
                    max={6}
                    value={riseTime}
                    onChange={(e) => setRiseTime(clamp(Number(e.target.value), 1, 6))}
                    className={`rounded-xl px-3 py-2 border w-24 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-700'}`}
                  />
                  <button
                    className={`px-3 py-2 rounded-xl ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-700'} border ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <span className="text-xl font-medium dark:text-white">S Mode</span>
              </div>
            </div>
            {lastSaved && (
